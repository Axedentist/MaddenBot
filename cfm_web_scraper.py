...
def cfm_web_scraper(link):
    ...
    '''
    This function takes a Madden Connected Franchise Link and grabs the most relevant information for people who are putting players on a trade block.
    It will be used in a Discord bot for the Franchise. 
    '''
    ...
    # Scrape the classes that give us the player's general information (name, height, weight etc.) 
    # as well as contract information (salary, total years, years left, cap hit etc.) 
    ...
    # split on those spaces and remove those from the list, leaving only the info we want.
    ...
    # Here we are grabbing information from the contract_info_list and player_info_list and will put them into an empty dictionary.
    # All but one player will fall into the elif or else statement, but Ha Ha Clinton-Dix was a special case, so he was given his own if statement.
    if player_info_list[0] == 'Ha':
        d['Name'] = player_info_list[0] + ' ' + player_info_list[1] + ' ' + player_info_list[2]
        d['Position'] = player_info_list[3]
        d['Overall'] = player_info_list[5]
        d['Height'] = player_info_list[7]
        d['Weight'] = player_info_list[9]
        d['Age'] = player_info_list[11]
        d['Years in League'] = player_info_list[13]
        d['Dev'] = player_info_list[18]
        d['Trade Value'] = player_info_list[20]
